...
export const BatchesDetailView = ({
    ...
    return (
      <div className="min-h-screen bg-[#0A0A0A] flex flex-col">
        ...
        <main className="flex-1 flex flex-col items-center justify-center p-8 text-neutral-400">
          ...
          <h2 className="text-lg font-semibold text-neutral-200">Batch não encontrado</h2>
          <p className="text-sm">O lote que você procura não existe ou foi removido.</p>
        </main>
      </div>
    );
  }
  ...
  const isExpired = batch.expirationDate ? isPast(new Date(batch.expirationDate)) && !isToday(new Date(batch.expirationDate)) : false;
  const statusColor = isExpired ? "text-rose-500" : "text-emerald-500";
  const statusBg = isExpired ? "bg-rose-500/10" : "bg-emerald-500/10";
  const statusBorder = isExpired ? "border-rose-500/20" : "border-emerald-500/20";
          ...
          <div className="flex items-center gap-2">
            ...
            <AlertDialog open={isDeleteOpen} onOpenChange={onDeleteOpenChange}>
              ...
              <AlertDialogContent className="rounded-[4px] border-[#262626] bg-[#171717]">
                <AlertDialogHeader>
                  <AlertDialogTitle className="text-white">Excluir batch permanentemente?</AlertDialogTitle>
                  <AlertDialogDescription className="text-neutral-400">
                    Esta ação não pode ser desfeita. Isso excluirá permanentemente o batch e removerá os dados de nossos servidores.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  <AlertDialogCancel className="rounded-[4px] border-[#262626] bg-transparent text-white hover:bg-[#262626]">Cancelar</AlertDialogCancel>
                  <AlertDialogAction
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="group relative overflow-hidden rounded-[4px] border border-[#262626] bg-[#171717]">
               ...
               <div className="p-6">
                  <div className="flex items-start justify-between mb-6">
                    <div>
                      <h3 className="text-xs font-bold uppercase tracking-widest text-blue-500 mb-1">Produto Vinculado</h3>
                      <Link href={`/products/${batch.productId}`} className="hover:underline decoration-blue-500/50 underline-offset-4">
                        <h2 className="text-2xl font-semibold text-white tracking-tight">{batch.productName}</h2>
                      </Link>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-4 border-t border-[#262626]">
                    <div>
                      <p className="text-[10px] uppercase tracking-wider text-neutral-500 mb-1">SKU do Produto</p>
                      <p className="font-mono text-sm text-neutral-200">{batch.productSku || "---"}</p>
                    </div>
                    <div>
                       <p className="text-[10px] uppercase tracking-wider text-neutral-500 mb-1">Categoria</p>
                       <p className="text-sm text-neutral-200">Geral</p> {/* Placeholder if category not in batch object */}
                    </div>
                  </div>
               </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <div className="rounded-[4px] border border-[#262626] bg-[#171717] p-5">
                  <div className="flex items-center gap-2 mb-4 text-emerald-500">
                    ...
                    <h3 className="text-xs font-bold uppercase tracking-widest">Financeiro</h3>
                  </div>
                  
                  <div className="space-y-4">
                    <div className="flex justify-between items-end border-b border-[#262626] pb-2">
                      <span className="text-xs text-neutral-500">Preço de Custo</span>
                      <span className="font-mono text-sm text-white">{formatCurrency(batch.costPrice)}</span>
                    </div>
                    <div className="flex justify-between items-end border-b border-[#262626] pb-2">
                      <span className="text-xs text-neutral-500">Preço de Venda</span>
                      <span className="font-mono text-sm text-white">{formatCurrency(batch.sellingPrice)}</span>
                    </div>
                    <div className="flex justify-between items-end pt-1">
                      <span className="text-xs text-neutral-500">Margem Estimada</span>
                      {batch.sellingPrice && batch.costPrice ? (
                        <span className="font-mono text-sm text-emerald-500 font-bold">
                          {Math.round(((batch.sellingPrice - batch.costPrice) / batch.costPrice) * 100)}%
                        </span>
                      ) : (
               ...
               <div className="rounded-[4px] border border-[#262626] bg-[#171717] p-5">
                  ...
                  <div className="space-y-1">
                    <p className="text-xs text-neutral-500 uppercase tracking-wide">Warehouse</p>
                    <p className="text-lg font-medium text-white">{batch.warehouseName}</p>
                    <div className="pt-2">
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="rounded-[4px] border border-[#262626] bg-[#171717] p-6 relative overflow-hidden">
              ...
              <h3 className="text-xs font-bold uppercase tracking-widest text-neutral-500 mb-4">Estoque Atual</h3>
              
              <div className="flex items-baseline gap-1">
                 <span className="text-5xl font-bold tracking-tighter text-white">{batch.quantity}</span>
                 <span className="text-sm text-neutral-500 font-medium">unid.</span>
              </div>
            ...
            <div className="rounded-[4px] border border-[#262626] bg-[#171717] p-6">
               <div className="flex items-center gap-2 mb-6 text-amber-500">
                  ...
                  <h3 className="text-xs font-bold uppercase tracking-widest">Ciclo de Vida</h3>
               </div>
               ...
               <div className="relative pl-4 space-y-8 border-l border-[#262626]">
                  ...
                  <div className="relative">
                    ...
                    <p className="text-[10px] uppercase tracking-wider text-neutral-500 mb-0.5">Fabricação</p>
                    <p className="text-sm font-medium text-white">{formatDate(batch.manufacturedDate)}</p>
                  </div>

                  {/* Created At */}
                  <div className="relative">
                    <div className="absolute -left-[21px] top-1 h-2.5 w-2.5 rounded-full border-2 border-[#171717] bg-blue-600" />
                    <p className="text-[10px] uppercase tracking-wider text-neutral-500 mb-0.5">Registrado em</p>
                    <p className="text-sm font-medium text-white">{formatDate(batch.createdAt)}</p>
                  </div>
                  ...
                  <div className="relative">
                    ...
                    <p className={cn("text-[10px] uppercase tracking-wider mb-0.5", isExpired ? "text-rose-500 font-bold" : "text-neutral-500")}>
                      Validade
                    </p>
                    <div className="flex items-center gap-2">
                      <p className={cn("text-sm font-medium", isExpired ? "text-rose-400" : "text-white")}>
