...
async function fetchProductPromptAsset(assetUrl: URL): Promise<Response> {
  try {
    ...
    if (!response.ok) return createProductPromptAssetErrorResponse("Imagem indisponível.", 502);
...
async function createProductPromptAssetResponse(
  ...
  if (!contentType.startsWith("image/")) {
    return createProductPromptAssetErrorResponse("Arquivo remoto não é imagem.", 415);
  }
